...
class Data:
    def __init__(self, data_dir='./data/JF17K/', max_vis_len_ent = 1):
        ...
        self.num_rel, self.num_ent = len(self.rel2id), len(self.ent2id)
        ...
        self.max_vis_len_ent = max_vis_len_ent
        ...
        self.gather_vis_feature(data_dir)
        self.gather_txt_feature(data_dir)




    def gather_vis_feature(self, data_dir='./data/JF17K/'):
        dataset = data_dir.strip().split('/')[-2]
        if dataset=='JF17K-3' or dataset=='JF17K-4':
            file_in = './data/JF17K/image_vectors_JF17K.npy'
        elif dataset=='WikiPeople-3' or dataset=='WikiPeople-4':
            file_in = './data/WikiPeople/image_vectors_WikiPeople.npy'
        else:
            file_in = data_dir + 'image_vectors_' + dataset + '.npy'

        if os.path.isfile(file_in):
            self.ent2vis = np.load(file_in, allow_pickle=True).item()
        else:
            self.ent2vis = {}
        ...
        if self.max_vis_len_ent != -1:
            for ent_name in self.ent2vis:
                ...
                if self.ent2id.get(ent_name) is None:
                    continue
                # if ent_2_id.get(self.ent2id[ent_name]) is not None:
                #     total_num_2 = total_num_2 + num_feats
                total_num += num_feats
        ...
        for ent_name in self.ent2vis:
            ...
            self.ent_vis_matrix[ent_id, :num_feats] = torch.from_numpy(self.ent2vis[ent_name][0])


    def gather_txt_feature(self, data_dir='./data/JF17K/'):
        dataset = data_dir.strip().split('/')[-2]
        if dataset=='JF17K-3' or dataset=='JF17K-4':
            file_in = './data/JF17K/text_vectors_JF17K.npy'
        elif dataset=='WikiPeople-3' or dataset=='WikiPeople-4':
            file_in = './data/WikiPeople/text_vectors_WikiPeople.npy'
        else:
            file_in = data_dir + 'text_vectors_' + dataset + '.npy'
        self.ent2txt = np.load(file_in, allow_pickle=True).item()
        self.txt_feat_size = self.ent2txt.popitem()[1].shape[0]


        self.ent_txt_matrix = torch.zeros((self.num_ent, self.txt_feat_size)).cuda()

        self.ent_txt_mask = torch.full((self.num_ent, 1), True).cuda()
        self.ent_txt_matrix = torch.zeros((self.num_ent, 1, self.txt_feat_size)).cuda()

        total_num = 0
        for ent_name in self.ent2txt:
            if self.ent2id.get(ent_name) is None:
                continue

            total_num += 1
            ent_id = self.ent2id[ent_name]
            num_feats = len(self.ent2txt[ent_name])
            self.ent_txt_mask[ent_id, :num_feats] = False
            self.ent_txt_matrix[ent_id, :num_feats] = torch.from_numpy(self.ent2txt[ent_name])
